...
exports.setServicePrice = async (req, res) => {
  try {
    const { service_id, car_model_id, price, note, is_active } = req.body;

    // ถ้ามี is_active (เช่น ลบข้อมูล) ให้ update เฉพาะ is_active
    if (typeof is_active !== 'undefined') {
      const [updated] = await ServicePricing.update(
        { is_active },
        { where: { service_id, car_model_id } }
      );
      return res.json({ message: updated ? 'อัปเดตสถานะเรียบร้อย' : 'ไม่พบข้อมูล', updated });
    }
