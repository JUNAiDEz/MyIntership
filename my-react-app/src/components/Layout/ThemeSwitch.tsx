...
// สวิตช์สลับธีม (Uiverse widget) — สไตล์/อนิเมชันเก็บไว้ใน ThemeSwitch.module.css
function ThemeSwitch() {
  // 1. แก้ไขให้ค่าเริ่มต้นเป็น true (เปิดสวิตช์ธีมมืดรอไว้เลย)
  ...
  useEffect(() => {
    try {
      ...
      // 2. ถ้าเพิ่งเข้าเว็บครั้งแรก (saved เป็น null) หรือเคยบันทึกไว้ว่า '1' ให้เป็นธีมมืด
      if (saved === null || saved === '1') {
        setChecked(true);
        document.documentElement.classList.add('dark-theme');
        // บันทึกค่าเริ่มต้นลงเครื่องผู้ใช้เลยว่าตอนนี้เป็นโหมดมืด
        if (saved === null) {
          localStorage.setItem('theme-dark', '1');
        }
      } else {
        // ถ้าผู้ใช้เคยตั้งใจกดปิดเป็นธีมสว่าง (saved === '0') ก็เคารพการตัดสินใจเขา
        setChecked(false);
    ...
    <label className={`${styles['theme-switch']} ${styles.compact}`} aria-label="Toggle theme">
      ...
      <div className={styles['theme-switch__container']}>
        ...
        <div className={styles['theme-switch__stars-container']}>
          {/* ... SVG และโค้ดส่วนแสดงผลสวิตช์ด้านล่าง คงไว้เหมือนเดิม 100% ครับ ... */}
