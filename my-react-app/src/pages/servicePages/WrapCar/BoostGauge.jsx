...
const faqItems = [
  { question: 'Boost Gauge คืออะไร?', answer: 'คือเครื่องมือวัดแรงดันอากาศในระบบอัดอากาศ (Turbo) เพื่อเช็คและปรับแต่งการทำงานของเครื่องยนต์' },
  { question: 'ควรติดตั้งเกจวัดบูสเมื่อไร?', answer: 'ควรติดตั้งเมื่อมีการอัพเกรดเทอร์โบ หรือปรับแต่งระบบอัดอากาศ เพื่อความแม่นยำและความปลอดภัย' },
  { question: 'มีรับประกันไหม?', answer: 'รับประกันสินค้าและงานติดตั้ง 1 ปีเต็ม' }
];

// ================== MAIN COMPONENT ==================

function BoostGauge({ onLogout }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const res = await api.apiGet('/api/services/boost-gauge/pricing');
        ...
        console.error("Error fetching BoostGauge data:", err);
  ...
  const pageUrl = typeof window !== 'undefined' ? window.location.href : 'https://front.gt7dev.com/services/boost-gauge';
  const title = 'Boost Gauge — GT7 Motor';
  const description = 'บริการติดตั้งเกจวัดบูส (Boost Gauge) ทั้งดิจิตอลและอนาล็อก พร้อมเซ็ตค่าตามสเปค โดยช่างผู้เชี่ยวชาญ';
    ...
    <div className={styles.pageContainer}>
      ...
      <main className={styles.mainContent}>
        ...
        <HeroSection 
          bgImage={heroImage} 
          title="BOOST GAUGE" 
          subtitle="ติดตั้งเกจวัดบูส มาตรฐานแม่นยำ" 
        />
