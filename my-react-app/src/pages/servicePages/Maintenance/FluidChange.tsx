...
function FluidChange({ onLogout }: { onLogout?: () => void }) {
  ...
  const { data: dbData = [], isLoading: loadingPricing, isError: errorPricing } = useQuery({
    queryKey: ['service-pricing', 'fluid-change'],
    queryFn: async () => {
      const res = await api.apiGet<ServicePricingGroup[]>('/api/services/fluid-change/pricing');
