...
const reviewItems = [
    {
      ...
      title: 'เปลี่ยนโช๊คอัพใหม่ 4 ต้น',
      desc: 'จบปัญหารถย้วย โยนตัว เข้าโค้งไม่มั่นใจ'
    },
    {
      type: 'image',
      src: 'https://images.unsplash.com/photo-1555626040-d63c2200dc27?q=80&w=1000&auto=format&fit=crop',
      title: 'ติดตั้งสตรัทปรับเกลียว',
      desc: 'จัดทรงสวย โหลดเตี้ย ปรับนุ่มแข็งได้ตามใจ'
    },
	{
	  type: 'video',
	  videoId: 'dQw4w9WgXcQ', 
	  title: 'อาการโช๊คอัพรั่ว/เสีย',
	  desc: 'วิธีสังเกตอาการโช๊คอัพเสื่อมสภาพด้วยตัวเอง'
	}
];

function ShockAbsorber({ onLogout }) {
  ...
  useEffect(() => {
    const fetchPricing = async () => {
      try {
        ...
        const res = await api.apiGet('/api/services/shock-absorber/pricing');
  ...
  const heroImage = 'https://images.unsplash.com/photo-1486262715619-01b8c2297609?q=80&w=1600&auto=format&fit=crop';
  const pageUrl = typeof window !== 'undefined' ? window.location.href : 'https://front.gt7dev.com/services/fitment/shock-absorber';
  const title = 'Shock Absorber Service — เปลี่ยนโช๊คอัพ';
  const description = 'บริการเปลี่ยนโช๊คอัพมาตรฐานศูนย์ ด้วยอะไหล่คุณภาพ ราคามาตรฐาน พร้อมรีวิวจริงจากลูกค้า';
  const mainImage = heroImage;

  const prices = allServicePackages.map(p => {
    const digits = String(p.frontPair || p.rearPair || '').replace(/[^0-9]/g, '');
    ...
    <div className={styles.pageContainer}>
      ...
      <main className={styles.mainContent}>
        <HeroSection 
          ...
          subtitle="เปลี่ยนโช๊คอัพ นุ่มนวล เกาะถนน ปลอดภัยทุกการขับขี่" 
        />
