...
const reviewItems = [
    {
      type: 'image',
      src: 'https://images.unsplash.com/photo-1578844251758-2f71da645217?q=80&w=1000&auto=format&fit=crop',
      title: 'เปลี่ยนยางใหม่ 4 เส้น',
      desc: 'พร้อมถ่วงล้อและตั้งศูนย์ฟรี ขับขี่นุ่มเงียบ'
    },
    {
      type: 'image',
      src: 'https://images.unsplash.com/photo-1616422285623-13ff0162193c?q=80&w=1000&auto=format&fit=crop',
      title: 'จัดทรงล้อแม็กซ์',
      desc: 'ติดตั้งล้อแม็กซ์ลายสวย พร้อมยางสปอร์ต'
    },
	{
	  type: 'video',
	  videoId: 'dQw4w9WgXcQ', 
	  title: 'การถ่วงล้อมาตรฐาน',
	  desc: 'ขั้นตอนการถ่วงล้อและตั้งศูนย์ด้วยระบบคอมพิวเตอร์'
	}
];

function WheelsTires({ onLogout }: { onLogout?: () => void }) {
  ...
  const { data: dbData = [], isLoading: loadingPricing, isError: errorPricing } = useQuery({
    queryKey: ['service-pricing', 'wheels-tires'],
    queryFn: async () => {
      const res: any = await api.apiGet('/api/services/wheels-tires/pricing');
  ...
  const heroImage = 'https://images.unsplash.com/photo-1578844251758-2f71da645217?q=80&w=1600&auto=format&fit=crop';
  const pageUrl = typeof window !== 'undefined' ? window.location.href : 'https://front.gt7dev.com/services/fitment/wheels-tires';
  const title = 'Wheels & Tires — ล้อแม็กซ์ ยางรถยนต์';
  const description = 'บริการล้อแม็กซ์ ยางรถยนต์ คุณภาพ พร้อมถ่วงล้อและตั้งศูนย์ ราคามาตรฐาน พร้อมรีวิวจริงจากลูกค้า';
  const mainImage = heroImage;

  const prices = allServicePackages.map((p: any) => {
    const digits = String(p.tire || p.fullSet || '').replace(/[^0-9]/g, '');
    ...
    <div className={styles.pageContainer}>
      ...
      <main className={styles.mainContent}>
        <HeroSection 
          bgImage={heroImage} 
          title="WHEELS & TIRES" 
          subtitle="ยางคุณภาพ ล้อแม็กซ์สวย พร้อมบริการถ่วงล้อและตั้งศูนย์ครบวงจร" 
        />
