...
const reviewItems: ReviewItem[] = [
  { type: 'image', src: 'https://images.unsplash.com/photo-1519125323398-675f0ddb6308?q=80&w=1000&auto=format&fit=crop', title: 'ตั้งวาล์ว', desc: 'ตัวอย่างงานตั้งวาล์วเครื่องยนต์ เพื่อคืนกำลังอัดให้เครื่องยนต์' },
  { type: 'image', src: 'https://images.unsplash.com/photo-1549924231-f129b911e442?q=80&w=1000&auto=format&fit=crop', title: 'เจียร์บ่าวาล์ว', desc: 'ซ่อมบ่าวาล์วรั่ว แก้ปัญหาเครื่องยนต์เดินไม่เรียบ' },
  { type: 'video', videoId: 'QwZT7T-TXT0', title: 'รีวิวการแก้วาล์ว', desc: 'ขั้นตอนการแก้ไขวาล์วและบ่าวาล์วอย่างละเอียด' }
];

const faqItems = [
  { question: 'การแก้วาล์วคืออะไร?', answer: 'คือการตั้งระยะวาล์ว เจียร์บ่าวาล์ว เปลี่ยนยางตีนวาล์ว หรือซ่อมแซมส่วนที่เกี่ยวข้องกับวาล์ว เพื่อให้เครื่องยนต์ทำงานเต็มประสิทธิภาพ' },
  { question: 'ควรตั้งวาล์วบ่อยแค่ไหน?', answer: 'ควรตั้งวาล์วทุก 40,000-60,000 กม. หรือเมื่อมีเสียงดังผิดปกติ' },
  { question: 'อาการวาล์วรั่วเป็นอย่างไร?', answer: 'เครื่องเดินเบาสั่น กำลังตก หรือมีควันขาว' },
  { question: 'มีรับประกันไหม?', answer: 'รับประกันงานซ่อม 6 เดือน' }
];

// ================== MAIN COMPONENT ==================

function ValveService({ onLogout }: { onLogout?: () => void }) {
  ...
  const { data: dbData = [], isLoading: loadingPricing, isError: errorPricing } = useQuery({
    queryKey: ['service-pricing', 'valve-service'],
    queryFn: async () => {
      const res = await api.apiGet<ServicePricingGroup[]>('/api/services/valve-service/pricing');
  ...
  const heroImage = 'https://images.unsplash.com/photo-1519125323398-675f0ddb6308?q=80&w=1600&auto=format&fit=crop';
  const pageUrl = typeof window !== 'undefined' ? window.location.href : 'https://front.gt7dev.com/services/valve-service';
  const title = 'Valve Service — GT7 Motor';
  const description = 'บริการแก้ไขวาล์ว ตั้งวาล์ว เจียร์บ่าวาล์ว เปลี่ยนยางตีนวาล์ว ครบวงจร โดยช่างผู้เชี่ยวชาญ';
    ...
    <div className={styles.pageContainer}>
      ...
      <main className={styles.mainContent}>
        ...
        <HeroSection 
          bgImage={heroImage} 
          title="VALVE SERVICE" 
          subtitle="บริการแก้ไขวาล์วครบวงจร" 
        />
