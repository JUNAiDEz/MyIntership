// src/pages/Dashboard/UpgradeManagement/CustomExhaustManagementPage.jsx
...
export default function CustomExhaustManagementPage({ onLogout }: { onLogout?: () => void }) {
  const queryClient = useQueryClient();

  // --- List (React Query) ---
  const { data: items = [], isLoading: loading } = useQuery({
    queryKey: ['custom-exhaust-pricing'],
    queryFn: async () => {
      const res = await api.apiGet('/api/services/custom-exhaust/pricing');
  ...
  const refresh = () => queryClient.invalidateQueries({ queryKey: ['custom-exhaust-pricing'] });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const serviceRes = await api.apiGet<{ service_id: number }[]>('/api/services?search=Custom Exhaust');
      const service = (serviceRes && serviceRes.length > 0) ? serviceRes[0] : null;
      if (!service) {
        throw new Error('ไม่พบบริการ Custom Exhaust ในระบบ');
      }
      const body = {
      ...
      <div className={styles.pageContainer} style={{ flex: 1 }}>
        ...
        <div className={styles.header} style={{ display:'block' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                <h2 className={styles.title}>Custom Exhaust Management</h2>
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className={styles.formGroup}>
                            ...
                            <input
                                ...
                                value={formData.note}
                                onChange={handleChange}
                                placeholder="e.g. Full system + Installation"
                            />
                        </div>
