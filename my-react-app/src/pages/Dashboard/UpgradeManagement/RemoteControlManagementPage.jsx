...
export default function RemoteControlManagementPage({ onLogout }) {
  // --- State ---
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const serviceRes = await api.apiGet('/api/services?search=Remote Control');
      const service = (serviceRes && serviceRes.length > 0) ? serviceRes[0] : null;
      if (!service) {
        alert('ไม่พบบริการ Remote Control ในระบบ');
        return;
      }
      const body = {
