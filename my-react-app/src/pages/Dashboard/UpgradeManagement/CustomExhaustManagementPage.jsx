...
export default function CustomExhaustManagementPage({ onLogout }) {
  // --- State ---
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const serviceRes = await api.apiGet('/api/services?search=Custom Exhaust');
      const service = (serviceRes && serviceRes.length > 0) ? serviceRes[0] : null;
      if (!service) {
        alert('ไม่พบบริการ Custom Exhaust ในระบบ');
        return;
      }
      const body = {
