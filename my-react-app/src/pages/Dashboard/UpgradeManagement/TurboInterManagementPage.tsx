// src/pages/Dashboard/UpgradeManagement/TurboInterManagementPage.jsx
...
export default function TurboInterManagementPage({ onLogout }: { onLogout?: () => void }) {
  const queryClient = useQueryClient();

  // --- List (React Query) ---
  const { data: items = [], isLoading: loading } = useQuery({
    queryKey: ['turbo-inter-pricing'],
    queryFn: async () => {
      const res: any = await api.apiGet('/api/services/turbo-inter/pricing');
      const flat: any[] = [];
  ...
  const refresh = () => queryClient.invalidateQueries({ queryKey: ['turbo-inter-pricing'] });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const serviceRes: any = await api.apiGet('/api/services?search=Turbo Inter');
      const service = (serviceRes && serviceRes.length > 0) ? serviceRes[0] : null;
      if (!service) {
        throw new Error('ไม่พบบริการ Turbo Inter ในระบบ');
      }
      const body = {
      ...
      <div className={styles.pageContainer} style={{ flex: 1 }}>
        ...
        <div className={styles.header} style={{ display:'block' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                <h2 className={styles.title}>Turbo/Inter Management</h2>
            ...
            <div className={styles.modalBackdrop}>
                <div className={styles.modalContent}>
                    <div className={styles.modalHeader}>
                        <h3>{modal.mode === 'create' ? 'Add Turbo/Inter Price' : 'Edit Price'}</h3>
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className={styles.formGroup}>
                            ...
                            <input
                                ...
                                value={formData.note}
                                onChange={handleChange}
                                placeholder="e.g. Turbo Upgrade / Intercooler Kit"
                            />
                        </div>
