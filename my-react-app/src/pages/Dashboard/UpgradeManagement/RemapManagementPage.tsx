// src/pages/Dashboard/UpgradeManagement/RemapManagementPage.jsx
...
export default function RemapManagementPage({ onLogout }: { onLogout?: () => void }) {
  const queryClient = useQueryClient();

  // --- List (React Query) ---
  const { data: items = [], isLoading: loading } = useQuery({
    queryKey: ['remap-pricing'],
    queryFn: async () => {
  ...
  const refresh = () => queryClient.invalidateQueries({ queryKey: ['remap-pricing'] });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const serviceRes = await api.apiGet<{ service_id: number }[]>('/api/services?search=Remap');
      const service = (serviceRes && serviceRes.length > 0) ? serviceRes[0] : null;
      if (!service) {
        throw new Error('ไม่พบบริการ Remap ในระบบ');
      }
      const body = {
      ...
      <div className={styles.pageContainer} style={{ flex: 1 }}>
        ...
        <div className={styles.header} style={{ display:'block' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                <h2 className={styles.title}>Remap Management</h2>
            ...
            <div className={styles.modalBackdrop}>
                <div className={styles.modalContent}>
                    <div className={styles.modalHeader}>
                        <h3>{modal.mode === 'create' ? 'Add Remap Price' : 'Edit Price'}</h3>
