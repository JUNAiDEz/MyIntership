// src/pages/Dashboard/UpgradeManagement/ValveServiceManagementPage.jsx
...
export default function ValveServiceManagementPage({ onLogout }: { onLogout?: () => void }) {
  const queryClient = useQueryClient();

  // --- List (React Query) ---
  const { data: items = [], isLoading: loading } = useQuery({
    queryKey: ['valve-service-pricing'],
    queryFn: async () => {
  ...
  const refresh = () => queryClient.invalidateQueries({ queryKey: ['valve-service-pricing'] });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const serviceRes = await api.apiGet<{ service_id: number }[]>('/api/services?search=Valve Service');
      const service = (serviceRes && serviceRes.length > 0) ? serviceRes[0] : null;
      if (!service) {
        throw new Error('ไม่พบบริการ Valve Service ในระบบ');
      }
      const body = {
      ...
      <div className={styles.pageContainer} style={{ flex: 1 }}>
        ...
        <div className={styles.header} style={{ display:'block' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                <h2 className={styles.title}>Valve Service Management</h2>
            ...
            <div className={styles.modalBackdrop}>
                <div className={styles.modalContent}>
                    <div className={styles.modalHeader}>
                        <h3>{modal.mode === 'create' ? 'Add Valve Service Price' : 'Edit Price'}</h3>
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className={styles.formGroup}>
                            ...
                            <input
                                ...
                                value={formData.note}
                                onChange={handleChange}
                                placeholder="e.g. Valve clearance adjustment"
                            />
                        </div>
