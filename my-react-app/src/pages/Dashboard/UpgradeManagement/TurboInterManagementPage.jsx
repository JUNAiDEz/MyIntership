...
export default function TurboInterManagementPage({ onLogout }) {
  // --- State ---
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const serviceRes = await api.apiGet('/api/services?search=Turbo Inter');
      const service = (serviceRes && serviceRes.length > 0) ? serviceRes[0] : null;
      if (!service) {
        alert('ไม่พบบริการ Turbo Inter ในระบบ');
        return;
      }
      const body = {
