// src/pages/Dashboard/WrapCarManagement/StickerManagementPage.jsx
...
export default function StickerManagementPage({ onLogout }: { onLogout?: () => void }) {
  ...
  const { data: items = [], isLoading: loading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.apiGet<PricingGroup[]>('/api/services/sticker/pricing');
  ...
  const refresh = () => queryClient.invalidateQueries({ queryKey: ['sticker-pricing'] });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const serviceRes = await api.apiGet<Array<{ service_id: number }>>('/api/services?search=Sticker');
      const service = (serviceRes && serviceRes.length > 0) ? serviceRes[0] : null;
      if (!service) {
        throw new Error('ไม่พบบริการ Sticker ในระบบ');
      }
      const body = {
      ...
      <div className={styles.pageContainer} style={{ flex: 1 }}>
        ...
        <div className={styles.header} style={{ display:'block' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                <h2 className={styles.title}>Sticker Management</h2>
            ...
            <div className={styles.modalBackdrop}>
                <div className={styles.modalContent}>
                    <div className={styles.modalHeader}>
                        <h3>{modal.mode === 'create' ? 'Add Sticker Price' : 'Edit Price'}</h3>
