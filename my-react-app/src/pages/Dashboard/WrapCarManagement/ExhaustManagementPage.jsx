...
export default function ExhaustManagementPage({ onLogout }) {
  // --- State ---
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await api.apiGet('/api/services/exhaust/pricing');
      const flat = [];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const serviceRes = await api.apiGet('/api/services?search=Exhaust');
      const service = (serviceRes && serviceRes.length > 0) ? serviceRes[0] : null;
      if (!service) {
        alert('ไม่พบบริการ Exhaust ในระบบ');
        return;
      }
      const body = {
