// src/pages/Dashboard/WrapCarManagement/ExhaustManagementPage.jsx
...
export default function ExhaustManagementPage({ onLogout }: { onLogout?: () => void }) {
  const queryClient = useQueryClient();

  // --- List (React Query) ---
  const { data: items = [], isLoading: loading } = useQuery({
    queryKey: ['exhaust-pricing'],
    queryFn: async () => {
      const res = await api.apiGet<PricingGroup[]>('/api/services/exhaust/pricing');
  ...
  const refresh = () => queryClient.invalidateQueries({ queryKey: ['exhaust-pricing'] });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const serviceRes = await api.apiGet<Array<{ service_id: number }>>('/api/services?search=Exhaust');
      const service = (serviceRes && serviceRes.length > 0) ? serviceRes[0] : null;
      if (!service) {
        throw new Error('ไม่พบบริการ Exhaust ในระบบ');
      }
      const body = {
      ...
      <div className={styles.pageContainer} style={{ flex: 1 }}>
        ...
        <div className={styles.header} style={{ display:'block' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                <h2 className={styles.title}>Exhaust Management (Standard)</h2>
            ...
            {/* Navigation Tabs (WrapCar Group - ตาม Route เดิม) */}
            ...
            <div className={styles.modalBackdrop}>
                <div className={styles.modalContent}>
                    <div className={styles.modalHeader}>
                        <h3>{modal.mode === 'create' ? 'Add Exhaust Price' : 'Edit Price'}</h3>
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className={styles.formGroup}>
                            ...
                            <input
                                ...
                                value={formData.note}
                                onChange={handleChange}
                                placeholder="e.g. Standard replacement"
                            />
                        </div>
