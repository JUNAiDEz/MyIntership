// src/pages/Dashboard/WrapCarManagement/BoostGaugeManagementPage.jsx
...
export default function BoostGaugeManagementPage({ onLogout }: { onLogout?: () => void }) {
  const queryClient = useQueryClient();

  // --- List (React Query) ---
  const { data: items = [], isLoading: loading } = useQuery({
    queryKey: ['boost-gauge-pricing'],
    queryFn: async () => {
      const res = await api.apiGet<PricingGroup[]>('/api/services/boost-gauge/pricing');
  ...
  const refresh = () => queryClient.invalidateQueries({ queryKey: ['boost-gauge-pricing'] });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const serviceRes = await api.apiGet<Array<{ service_id: number }>>('/api/services?search=Boost Gauge');
      const service = (serviceRes && serviceRes.length > 0) ? serviceRes[0] : null;
      if (!service) {
        throw new Error('ไม่พบบริการ Boost Gauge ในระบบ');
      }
      const body = {
      ...
      <div className={styles.pageContainer} style={{ flex: 1 }}>
        ...
        <div className={styles.header} style={{ display:'block' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                <h2 className={styles.title}>Boost Gauge Management</h2>
            ...
            <div className={styles.modalBackdrop}>
                <div className={styles.modalContent}>
                    <div className={styles.modalHeader}>
                        <h3>{modal.mode === 'create' ? 'Add Boost Gauge Price' : 'Edit Price'}</h3>
