// src/pages/Dashboard/FitmentManagement/WheelsTiresManagementPage.jsx
...
export default function WheelsTiresManagementPage({ onLogout }) {
  // --- State ---
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const serviceRes = await api.apiGet('/api/services?search=Wheels & Tires');
      const service = (serviceRes && serviceRes.length > 0) ? serviceRes[0] : null;
      if (!service) {
        alert('ไม่พบบริการ Wheels & Tires ในระบบ');
        return;
      }
      const body = {
