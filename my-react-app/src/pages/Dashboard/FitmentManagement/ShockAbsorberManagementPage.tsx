// src/pages/Dashboard/FitmentManagement/ShockAbsorberManagementPage.tsx
...
export default function ShockAbsorberManagementPage({ onLogout }: { onLogout?: () => void }) {
  // --- State ---
  ...
  const { data: items = [], isLoading: loading } = useQuery({
    queryKey: ['shock-absorber-pricing-admin'],
    queryFn: async () => {
  ...
  const refresh = () => queryClient.invalidateQueries({ queryKey: ['shock-absorber-pricing-admin'] });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const serviceRes = await api.apiGet<Array<{ service_id: number }>>('/api/services?search=Shock Absorber');
      const service = (serviceRes && serviceRes.length > 0) ? serviceRes[0] : null;
      if (!service) {
        throw new Error('ไม่พบบริการ Shock Absorber ในระบบ');
      }
      const body = {
      ...
      <div className={styles.pageContainer} style={{ flex: 1 }}>
        ...
        <div className={styles.header} style={{ display:'block' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                <h2 className={styles.title}>Shock Absorber Management</h2>
            ...
            <div className={styles.modalBackdrop}>
                <div className={styles.modalContent}>
                    <div className={styles.modalHeader}>
                        <h3>{modal.mode === 'create' ? 'Add Shock Absorber Price' : 'Edit Price'}</h3>
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className={styles.formGroup}>
                            ...
                            <input
                                ...
                                value={formData.note}
                                onChange={handleChange}
                                placeholder="e.g. Per pair / Per set"
                            />
                        </div>
