// src/pages/Dashboard/FitmentManagement/AlignmentManagementPage.tsx
...
export default function AlignmentManagementPage({ onLogout }: { onLogout?: () => void }) {
  // --- State ---
  ...
  const { data: items = [], isLoading: loading } = useQuery({
    queryKey: ['alignment-pricing-admin'],
    queryFn: async () => {
  ...
  const refresh = () => queryClient.invalidateQueries({ queryKey: ['alignment-pricing-admin'] });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const serviceRes = await api.apiGet<Array<{ service_id: number }>>('/api/services?search=Alignment');
      const service = (serviceRes && serviceRes.length > 0) ? serviceRes[0] : null;
      if (!service) {
        throw new Error('ไม่พบบริการ Alignment ในระบบ');
      }
      const body = {
      ...
      <div className={styles.pageContainer} style={{ flex: 1 }}>
        ...
        <div className={styles.header} style={{ display:'block' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                <h2 className={styles.title}>Alignment Management</h2>
            ...
            <div className={styles.modalBackdrop}>
                <div className={styles.modalContent}>
                    <div className={styles.modalHeader}>
                        <h3>{modal.mode === 'create' ? 'Add Alignment Price' : 'Edit Price'}</h3>
