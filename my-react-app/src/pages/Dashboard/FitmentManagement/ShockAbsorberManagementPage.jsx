// src/pages/Dashboard/FitmentManagement/ShockAbsorberManagementPage.jsx
...
export default function ShockAbsorberManagementPage({ onLogout }) {
  // --- State ---
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const serviceRes = await api.apiGet('/api/services?search=Shock Absorber');
      const service = (serviceRes && serviceRes.length > 0) ? serviceRes[0] : null;
      if (!service) {
        alert('ไม่พบบริการ Shock Absorber ในระบบ');
        return;
      }
      const body = {
