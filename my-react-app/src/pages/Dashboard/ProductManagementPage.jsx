...
function ProductManagementPage() {
  ...
  const [carModelModal, setCarModelModal] = useState({ open: false, productTemplateId: null });
  ...
  const fetchDropdowns = useCallback(async () => {
    try {
      ...
      if (catRes.ok) setCategories((await catRes.json()).items || []);
      if (brandRes.ok) setBrands((await brandRes.json()).items || []);
      if (supRes.ok) setSuppliers((await supRes.json()).items || []);
      if (typeRes.ok) setTypes((await typeRes.json()).items || []);
    } catch (err) { console.error(err); }
  }, [headers]);

  useEffect(() => { fetchProducts(); fetchDropdowns(); }, [fetchProducts, fetchDropdowns]);
  ...
  const handleAddCategory = async () => {
    ...
    try {
      ...
      setNewCategory('');
      const catRes = await fetch(`${API_URL}/api/inventory/categories`, { headers });
      if (catRes.ok) setCategories((await catRes.json()).items || []);
    } catch (err) { setManageError(err.message); }
    finally { setManageLoading(false); }
  };
  const handleDeleteCategory = async (id) => {
    ...
    try { await fetch(`${API_URL}/api/inventory/categories/${id}`, { method: 'DELETE', headers }); fetchDropdowns(); } catch (err) { alert('ลบไม่สำเร็จ'); }
  };
  const handleAddType = async () => {
    ...
    try {
      ...
      setNewType('');
      const typeRes = await fetch(`${API_URL}/api/inventory/types`, { headers });
      if (typeRes.ok) setTypes((await typeRes.json()).items || []);
    } catch (err) { setManageError(err.message); }
    finally { setManageLoading(false); }
  };
  const handleDeleteType = async (id) => {
    ...
    try { await fetch(`${API_URL}/api/inventory/types/${id}`, { method: 'DELETE', headers }); fetchDropdowns(); } catch (err) { alert('ลบไม่สำเร็จ'); }
  };
  const handleAddBrand = async () => {
    ...
    try {
      ...
      setNewBrand('');
      const brandRes = await fetch(`${API_URL}/api/inventory/brands`, { headers });
      if (brandRes.ok) setBrands((await brandRes.json()).items || []);
    } catch (err) { setManageError(err.message); }
    finally { setManageLoading(false); }
  };
  const handleDeleteBrand = async (id) => {
    ...
    try { await fetch(`${API_URL}/api/inventory/brands/${id}`, { method: 'DELETE', headers }); fetchDropdowns(); } catch (err) { alert('ลบไม่สำเร็จ'); }
  };
      ...
      if (name === 'is_active' || name === 'is_popular') {
        ...
        if (modal.product && modal.product.id) {
          ...
          fetch(`${API_URL}/api/inventory/products/${modal.product.id}/flags`, {
            ...
            .then(res => res.ok ? fetchProducts() : res.json().then(err => Promise.reject(err)))
            .catch(err => setCreateError(err.message || 'บันทึกสถานะไม่สำเร็จ'));
        }
      } else {
  ...
  const getCategoryName = (id) => categories.find(c => c.category_id === Number(id))?.category_name || '-';
  ...
  return (
    <div className={styles.pageContainer}>
      ...
      <div className={styles.header} style={{ flexDirection: 'column', alignItems: 'stretch', gap: '15px' }}>
        ...
        <div className={styles.controls} style={{ background: '#f8f9fa', padding: '15px', borderRadius: '8px', border: '1px solid #e9ecef', display: 'flex', flexWrap: 'wrap', gap: '15px' }}>
          ...
          <div className={styles.searchWrapper} style={{ flex: 1, minWidth: '200px' }}>
            ...
            <input className={styles.searchInput} placeholder="Search Name, SKU..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
          </div>
          {/* Filters (Categories, Brand, Type) */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
             <label style={{ fontSize: '0.8rem', fontWeight: 600, color: '#555' }}>หมวดหมู่</label>
             <select className={styles.filterSelect} value={categoryFilter} onChange={e => { setCategoryFilter(e.target.value); setPage(1); }} style={{ margin: 0, minWidth: '160px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
             ...
             <select className={styles.filterSelect} value={typeFilter} onChange={e => { setTypeFilter(e.target.value); setPage(1); }} style={{ margin: 0, minWidth: '160px', cursor: 'pointer' }}>
                <option value="">ทั้งหมด</option>
                {types.map(t => ( <option key={t.product_type_id} value={t.product_type_id}>{t.type_name}</option> ))}
             </select>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
             <label style={{ fontSize: '0.8rem', fontWeight: 600, color: '#555' }}>แบรนด์</label>
             <select className={styles.filterSelect} value={brandFilter} onChange={e => { setBrandFilter(e.target.value); setPage(1); }} style={{ margin: 0, minWidth: '160px' }}>
                <option value="">ทั้งหมด</option>
      ...
      {/* Manage Data Modals... */}
      {manageModalOpen && (
        <div className={styles.modalBackdrop} onClick={e => { if(e.target === e.currentTarget) setManageModalOpen(false); }}>
              ...
              {manageTab==='category' && (
                <>
                  <div style={{display:'flex', gap:10, marginBottom:10}}>
                    <input className={styles.formInput} value={newCategory} onChange={e=>setNewCategory(e.target.value)} placeholder="เพิ่มหมวดหมู่ใหม่" />
                    <button className={styles.addButton} style={{padding:'0 20px'}} onClick={handleAddCategory} disabled={manageLoading}>เพิ่ม</button>
                  </div>
                    ...
                    {categories.map(cat => (
                      <li key={cat.category_id} style={{display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px', borderBottom:'1px solid #eee'}}>
                        {editCategoryId === cat.category_id ? (
                          <>
                            <input className={styles.formInput} value={editCategoryName} onChange={e=>setEditCategoryName(e.target.value)} style={{marginRight:8}} />
                            <button className={styles.addButton} style={{marginRight:8}} onClick={handleSaveCategory} disabled={manageLoading}>บันทึก</button>
      ...
      {supplierModalOpen && (
        <div className={styles.modalBackdrop}>
          <div className={styles.modalContent} style={{width: '90vw', maxWidth: 1100, minHeight: 600, position: 'relative'} }>
...
function ProductDetailModal({ 
    ...
    const fetchAuditLogs = async () => {
        if (modal.product?.id) {
            try {
                ...
                if (res.ok) {
                    const data = await res.json();
                    setAuditLogs((data.items || []).map(log => ({
                        ...log, 
    ...
    const labelStyle = { display: 'block', fontSize: '0.75rem', fontWeight: '800', color: '#6b7280', marginBottom: '6px', textTransform: 'uppercase' };

    return (
        <div className={styles.modalBackdrop} style={{ backdropFilter: 'blur(5px)' }} onClick={e => { if(e.target === e.currentTarget) setModal({...modal, open: false}); }}>
            <div style={{
                                    ...
                                    {isEditing && (
                                        <div style={{ position: 'absolute', bottom: 10, left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 10 }}>
                                            <label style={{ background: '#fff', padding: '6px 12px', borderRadius: 20, fontSize: '0.8rem', fontWeight: 'bold', cursor: 'pointer', boxShadow: '0 2px 5px rgba(0,0,0,0.2)', display: 'flex', alignItems: 'center', gap: 5 }}>
                                                <FaCamera /> เปลี่ยนรูป
                                                <input type="file" onChange={(e) => handleImageUpload(e.target.files[0])} style={{ display: 'none' }} />
                                            </label>
                                        </div>
                                    )}
                                </div>
