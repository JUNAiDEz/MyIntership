...
const ImageInputBox = ({ label, field, preview, onChange }) => (
...
function StickerDashboard() {
  ...
  const [modal, setModal] = useState({ open: false, type: 'car', mode: 'create', data: null });
  ...
  const [previews, setPreviews] = useState(initialCarState);
  ...
  useEffect(() => {
    Promise.all([fetchCars(), fetchColors()]).then(() => setLoading(false));
  }, []);

  const filteredCars = cars.filter(c => c.name.toLowerCase().includes(debouncedSearch.toLowerCase()));
  const filteredColors = colors.filter(c => c.name.toLowerCase().includes(debouncedSearch.toLowerCase()) || c.color_id.toLowerCase().includes(debouncedSearch.toLowerCase()));

  const handleCarFileChange = (e, field) => {
  ...
  const handleDeleteCar = async (id) => {
    ...
    try { await fetch(`${STICKER_API_URL}/cars/${id}`, { method: 'DELETE' }); fetchCars(); } catch (err) { alert(err.message); }
  };
  ...
  const handleColorSubmit = async (e) => {
    ...
    try {
      ...
      fetchColors(); closeModal();
    } catch (err) { alert(err.message); }
  };

  const handleDeleteColor = async (id) => {
    if (!window.confirm('Delete color?')) return;
    try { await fetch(`${STICKER_API_URL}/colors/${id}`, { method: 'DELETE' }); fetchColors(); } catch (err) { alert(err.message); }
  };
