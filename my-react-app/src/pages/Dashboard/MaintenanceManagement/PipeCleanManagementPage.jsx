...
export default function PipeCleanManagementPage({ onLogout }) {
  // --- State ---
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await api.apiGet('/api/services/pipe-clean/pricing');
      const flat = [];
      (res || []).forEach(group => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Logic หา Service ID
      const serviceRes = await api.apiGet('/api/services?search=Intake Cleaning');
      const service = (serviceRes && serviceRes.length > 0) ? serviceRes[0] : null;
      if (!service) {
        alert('ไม่พบบริการ Intake Cleaning ในระบบ');
        return;
      }
