// src/pages/Dashboard/MaintenanceManagement/EngineSpaManagementPage.jsx
...
export default function EngineSpaManagementPage() {
  // --- State ---
  ...
  const { data: items = [], isLoading: loading } = useQuery({
    queryKey: ['engine-spa-pricing-admin'],
    queryFn: async () => {
      const res = await api.apiGet<PricingGroup[]>('/api/services/engine-spa/pricing');
  ...
  const refresh = () => queryClient.invalidateQueries({ queryKey: ['engine-spa-pricing-admin'] });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const serviceRes = await api.apiGet<Array<{ service_id: number }>>('/api/services?search=Engine Spa');
      const service = (serviceRes && serviceRes.length > 0) ? serviceRes[0] : null;
      if (!service) {
        throw new Error('ไม่พบบริการ Engine Spa ในระบบ');
      }
      const body = {
    ...
    <div className={styles.pageContainer}>
      ...
      <div className={styles.header} style={{ display:'block' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h2 className={styles.title}>Engine Spa Management</h2>
          ...
          <div className={styles.modalBackdrop}>
              <div className={styles.modalContent}>
                  <div className={styles.modalHeader}>
                      <h3>{modal.mode === 'create' ? 'Add Engine Spa Price' : 'Edit Price'}</h3>
