// src/pages/Dashboard/PipeCleanManagementPage.jsx
import React, { useState, useEffect } from 'react';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import api from '../../../utils/api';
import styles from '../../../styles/AdminTheme.module.css';
import { FaEdit, FaTrash, FaPlus, FaSearch, FaTimes, FaSave, FaWrench, FaCar, FaBoxOpen, FaTags } from 'react-icons/fa';

// 🔥 Import DashboardHeader เข้ามาใช้งาน (เช็ค Path ให้ตรงด้วยนะครับ)
...
export default function PipeCleanManagementPage({ onLogout }: { onLogout?: () => void }) {
  // --- State ---
  ...
  // --- 1. Fetch Data Logic (React Query) ---
  const { data: items = [], isLoading: loading } = useQuery({
    queryKey: ['pipe-clean-pricing-admin'],
    queryFn: async () => {
      const res = await api.apiGet<PricingGroup[]>('/api/services/pipe-clean/pricing');
  ...
  const refresh = () => queryClient.invalidateQueries({ queryKey: ['pipe-clean-pricing-admin'] });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      // Logic หา Service ID
      const serviceRes = await api.apiGet<Array<{ service_id: number }>>('/api/services?search=Intake Cleaning');
      const service = (serviceRes && serviceRes.length > 0) ? serviceRes[0] : null;
      if (!service) {
        throw new Error('ไม่พบบริการ Intake Cleaning ในระบบ');
      }
  ...
  return (
    // เพิ่ม Layout คลุมทั้งหมดเหมือนหน้า Dealer
      ...
      <div className={styles.pageContainer} style={{ flex: 1 }}>
        ...
        <div className={styles.header} style={{ display:'block' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                <h2 className={styles.title}>Intake Cleaning Management</h2>
            ...
            <div className={styles.modalBackdrop}>
                <div className={styles.modalContent}>
                    <div className={styles.modalHeader}>
                        <h3>{modal.mode === 'create' ? 'Add Price' : 'Edit Price'}</h3>
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className={styles.formGroup}>
                            ...
                            <input
                                ...
                                value={formData.note}
                                onChange={handleChange}
                                placeholder="e.g. Include chemical wash"
                            />
                        </div>
