...
function CarManagementPage() {
  ...
  // --- Modal & Form ---
  const [modal, setModal] = useState({ open: false, mode: 'view', car: null });
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const mappedCars = (modelData.data || []).map(car => ({
        id: car.car_model_id,
  ...
  const availableYears = useMemo(() => {
    const years = cars.map(c => c.year).filter(y => y); // Get valid years
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setModal({ open: false, mode: 'view', car: null });
      fetchData();
    } catch (err) {
                ...
                >
                    ...
                    <div style={{ 
                        ...
                        <div style={{display:'flex', gap: '8px'}}>
                            ...
                            <button onClick={(e) => { e.stopPropagation(); openModal('view', c); }} className={styles.iconBtn} title="View"><FaEye /></button>
                            
                            <HasPermission resource="cars" action="update">
                                <button onClick={(e) => { e.stopPropagation(); openModal('edit', c); }} className={styles.iconBtn} title="Edit"><FaEdit /></button>
                            </HasPermission>
                            
                            <HasPermission resource="cars" action="delete">
                                <button onClick={(e) => { e.stopPropagation(); handleDelete(c.id); }} className={`${styles.iconBtn} ${styles.delete}`} title="Delete"><FaTrash /></button>
                            </HasPermission>
                        </div>
                    </div>
                </div>
              ...
              <div className={styles.formRow}>
                <div className={styles.formGroup}>
                  ...
                  <input 
                    ...
                    onChange={e => setCreateForm(f => ({ ...f, model_year: e.target.value }))}
                    disabled={modal.mode === 'view'}
                ...
                <div className={styles.formGroup}>
                  ...
                  <select 
                    ...
                    onChange={e => setCreateForm(f => ({ ...f, car_size: e.target.value }))}
                    disabled={modal.mode === 'view'}
                  >
                    <option value="SEDAN">SEDAN</option>
                    <option value="SUV">SUV / PPV</option>
