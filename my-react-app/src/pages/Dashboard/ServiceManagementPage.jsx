...
function ServiceManagementPage() {
    ...
    const handleAddCategory = async (e) => {
      ...
      setAddCatLoading(true); setAddCatError('');
      try {
        ...
        // Refresh categories
        const catsRes = await fetch(`${API_URL}/api/services/categories`, { headers });
        if (catsRes.ok) {
          const data = await catsRes.json();
          const mapped = (Array.isArray(data) ? data : data.items || []).map(c => ({ id: c.category_id || c.id, name: c.category_name || c.name }));
  ...
  const fetchServices = useCallback(async () => {
    ...
    try {
      ...
      const mapped = items.map((s) => ({
        ...
        category_name: categories.find(c => c.id === s.category_id)?.name || '-', 
        raw: s 
      }));
  ...
  }, [debouncedSearch, headers, categories]);
                ...
                <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.15)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <div style={{ background: '#fff', borderRadius: 10, padding: 30, minWidth: 320, boxShadow: '0 2px 16px rgba(0,0,0,0.12)' }}>
                    ...
                    <form onSubmit={handleAddCategory}>
                      <input type="text" value={newCategoryName} onChange={e => setNewCategoryName(e.target.value)} placeholder="ชื่อหมวดหมู่" style={{ width: '100%', padding: 8, borderRadius: 6, border: '1px solid #ccc', marginBottom: 12 }} autoFocus />
                      ...
                      <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                        ...
                        <button type="submit" disabled={addCatLoading} style={{ padding: '7px 18px', borderRadius: 6, border: 'none', background: '#ffc709', color: '#fff', fontWeight: 'bold', cursor: 'pointer' }}>{addCatLoading ? 'กำลังบันทึก...' : 'บันทึก'}</button>
                      </div>
                    </form>
                  </div>
                </div>
...
function ServiceDetailModal({ 
    ...
    const textareaStyle = { ...inputStyle, height: 'auto', minHeight: '80px' };
