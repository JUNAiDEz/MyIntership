    ...
    const fetchMasters = async () => {
      ...
      try {
        const [pRes, sRes] = await Promise.all([
          ...
          fetch(`${API_URL}/api/services?all=1`, { headers })
        ]);

        if (pRes.ok) {
          const pData = await pRes.json();
          setAllProducts(Array.isArray(pData) ? pData : pData.items || []);
        }
        if (sRes.ok) {
          const sData = await sRes.json();
          setAllServices(Array.isArray(sData) ? sData : sData.items || []);
        }
      } catch (e) {
      ...
      {modal.open && (
        <div className={styles.modalBackdrop}>
          <div className={styles.modalContent} style={{ width: '800px', maxWidth: '95%' }}>
            ...
            <form onSubmit={handleSubmit} disabled={modal.mode === 'view'}>
                  ...
                  <div className={styles.formRow}>
                      <div className={styles.formGroup}>
                        ...
                        <select 
                            ...
                            onChange={e => setCreateForm(f => ({ ...f, promotion_type: e.target.value }))}
                        >
                ...
                <div style={{flex:1, minWidth: '300px'}}>
                    ...
                    <div className={styles.formGroup}>
                        ...
                        <div style={{display:'flex', gap: 10}}>
                            ...
                            {modal.mode !== 'view' && (
                                <div style={{flex:1}}>
                                    <input type="file" accept="image/*" onChange={e => handleImageUpload(e.target.files[0])} style={{marginBottom:5, fontSize:'0.9rem'}} />
                ...
                {modal.mode !== 'view' && (
                    <div style={{display:'flex', gap:10, alignItems:'flex-end', background:'#f3f4f6', padding:15, borderRadius:8, flexWrap:'wrap'}}>
                        <div style={{flex:1, minWidth: '150px'}}>
                            ...
                            <select className={styles.formSelect} value={selServiceId} disabled={masterLoading || !!selProductId} onChange={e => { setSelServiceId(e.target.value); setSelProductId(''); }}>
                        ...
                        <div style={{flex:1, minWidth: '150px'}}>
                            ...
                            <select className={styles.formSelect} value={selProductId} disabled={masterLoading || !!selServiceId} onChange={e => {
                                setSelProductId(e.target.value);
                                setSelServiceId('');
                                setSelVariantId('');
                                setSelProductObj(allProducts.find(p => (p.id || p.product_template_id) == e.target.value));
                            }}>
                        ...
                        {selProductObj?.variants?.length > 0 && (
                            <div style={{flex:1, minWidth: '150px'}}>
                                ...
                                <select className={styles.formSelect} value={selVariantId} onChange={e => setSelVariantId(e.target.value)}>
